...
def do_stringcmp(cmp_method, str1, str2, min_threshold=None):
  ...
  if (cmp_method.startswith('exa')):
    ...
    start_time = time.time()
    sim_weight = sgram(str1, str2, [[0], [0, 1], [1, 2]], divisor,
                       min_threshold,
                       padded)
    time_used = time.time() - start_time
...
def winklermod(str1, str2, in_weight):
 ...
 """
  ...
  # Fixed based on: https://github.com/fgregg/febrl/pull/1
  # PC 18/09/2016
  #
  minlen = min(len(str1), len(str2))

  for same in range(min(4, minlen)):
...
def qgram(str1, str2, q=2, common_divisor='average', min_threshold=None,
  ...
  if (common_divisor == 'average'):
    divisor = 0.5 * (
          num_qgram1 + num_qgram2)  # Compute average number of q-grams
...
def posqgram(str1, str2, q=2, max_dist=2, common_divisor='average',
  ...
  # Make a list of q-grams for both strings - - - - - - - - - - - - - - - - - -
  #
  qgram_list1 = [(qgram_str1[i:i + q], i) for i in
                 range(len(qgram_str1) - (q - 1))]
  qgram_list2 = [(qgram_str2[i:i + q], i) for i in
                 range(len(qgram_str2) - (q - 1))]
...
def sgram(str1, str2, gc, common_divisor='average', min_threshold=None,
  ...
  for c in gc:
    ...
    if (common_divisor == 'average'):
      this_divisor = 0.5 * (
            num_sgram1 + num_sgram2)  # Average number of s-grams
    elif (common_divisor == 'shortest'):
...
def editdist_edits(str1, str2):
  ...
  if (str1 == '') and (str2 == ''):
    ...
    if (str1 == ''):
      ...
      return 0.0, [0, len(str1), 0,
                   0]  # Deletes nedded to get from str2 to empty
...
def swdist(str1, str2, common_divisor='average', min_threshold=None):
  ...
  # {b,p,v} -> 5
  #
  approx_matches = {'a': 0, 'b': 5, 'd': 1, 'e': 0, 'g': 2, 'i': 0, 'j': 2,
                    'l': 3,
                    'm': 4, 'n': 4, 'o': 0, 'p': 5, 'r': 3, 't': 1, 'u': 0,
                    'v': 5}
...
def compression(str1, str2, compressor='zlib', min_threshold=None):
  ...
  if (compressor == 'zlib'):
    ...
    c12 = 0.5 * (
          len(zlib.compress(str1 + str2)) + len(zlib.compress(str2 + str1)))

  elif (compressor == 'bz2'):
    c1 = float(len(bz2.compress(str1)))
    c2 = float(len(bz2.compress(str2)))
    c12 = 0.5 * (
          len(bz2.compress(str1 + str2)) + len(bz2.compress(str2 + str1)))
...
def ontolcs(str1, str2, min_common_len=2, common_divisor='average',
  ...
  for (s1, s2) in [(str1, str2), (str2, str1)]:
    ...
    h_diff += s1_len * s2_len / (
          P + (1 - P) * (s1_len + s2_len - s1_len * s2_len))
...
def editex(str1, str2, min_threshold=None):
  ...
  #
  groupsof_dict = {'a': 0, 'b': 1, 'c': 2, 'd': 3, 'e': 0, 'f': 1, 'g': 2,
                   'h': 7,
                   'i': 0, 'j': 2, 'k': 2, 'l': 4, 'm': 5, 'n': 5, 'o': 0,
                   'p': 1,
                   'q': 2, 'r': 6, 's': 2, 't': 3, 'u': 0, 'v': 1, 'w': 7,
                   'x': 2,
                   'y': 0, 'z': 2, '{': 7}
  ...
  for i in range(1, n + 1):
    ...
    for j in range(1, m + 1):
      ...
      F[i][j] = min(F[i - 1][j] + inc1, F[i][j - 1] + inc2,
                    F[i - 1][j - 1] + diag)
